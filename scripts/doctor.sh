...
MISSION="Mission 2.1: Weapon Handling Test"
...
if [ "${SDC_RUNNING:-0}" -gt 0 ]; then
    ...
    warn "SDC lab containers already running: ${THIS_LAB}" "If these belong to another mission, run 'make destroy' there first — this lab uses ports 2221-2223, 2241-2242"
else
    PORTS_BUSY=""
    for port in 2221 2222 2223 2241 2242; do
    ...
    if [ -n "$PORTS_BUSY" ]; then
        fail "Ports 2221-2223, 2241-2242 free" "Port(s)${PORTS_BUSY} in use — another mission lab or service is listening. Run 'make destroy' in the other mission, or free the port"
    else
        pass "Ports 2221-2223, 2241-2242 free"
    fi
fi
